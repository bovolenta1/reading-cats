...
export async function GET(req: Request) {
  ...
  if (error) {
    return NextResponse.redirect(new URL(`/login?error=${encodeURIComponent(error)}&desc=${encodeURIComponent(errorDesc ?? '')}`, url.origin));
  }
  ...
  if (!tokenRes.ok) {
    ...
    return NextResponse.redirect(new URL(`/login?error=token_exchange_failed&detail=${encodeURIComponent(txt)}`, url.origin));
  }
  ...
  const returnTo = url.searchParams.get('returnTo') ?? '/feed';
  const safePath = returnTo.startsWith('/') ? returnTo : '/feed';
