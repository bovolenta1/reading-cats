...
const CONFETTI_COLORS = ['#7d32f5', '#f6e434', '#63fdf1', '#e672da', '#295dfe', '#6e57ff'];
...
export default function PawLikeButton({ initialCount, className }: PawLikeButtonProps) {
  const [count, setCount] = useState(initialCount);
  const [liked, setLiked] = useState(false);
  const [phaseAnimation, setPhaseAnimation] = useState(false);
  const [phaseConfetti, setPhaseConfetti] = useState(false);
  const [confetti, setConfetti] = useState<ConfettiPiece[]>([]);
  const timeoutsRef = useRef<number[]>([]);

  const classes = useMemo(() => {
    const cls = ['paw-button'];
    if (phaseAnimation) cls.push('animation');
    if (phaseConfetti) cls.push('confetti');
    if (liked) cls.push('liked');
    if (className) cls.push(className);
    return cls.join(' ');
  }, [phaseAnimation, phaseConfetti, liked, className]);
  ...
  function startLike() {
    ...
    setPhaseAnimation(true);

    // confetti (igual ao pen)
    const pieces: ConfettiPiece[] = Array.from({ length: CONFETTI_AMOUNT }).map(() => ({
      id: uid(),
      x: randInt(-260, 260),
      y: randInt(-160, 160),
      r: randInt(0, 360),
      s: randInt(60, 100) / 100,
      b: CONFETTI_COLORS[randInt(0, CONFETTI_COLORS.length - 1)],
    }));
    setConfetti(pieces);

    timeoutsRef.current.push(
      window.setTimeout(() => {
        setPhaseConfetti(true);
  ...
  function undoLike() {
    clearTimers();
    setPhaseAnimation(false);
    setPhaseConfetti(false);
    setConfetti([]);
    setLiked(false);
  ...
  function onClick(e: React.MouseEvent<HTMLButtonElement>) {
    e.preventDefault();
    if (!phaseAnimation) startLike();
    else undoLike();
  }

  return (
    <>
      <button type="button" className={classes} onClick={onClick} aria-pressed={liked} aria-label="Like">
        <div className="text">
          <svg aria-hidden="true">
            <use href="#heart" xlinkHref="#heart" />
          </svg>
        </div>
        ...
        <div className="paws" aria-hidden="true">
          ...
          <svg className="paw-clap">
            <use href="#paw-clap" xlinkHref="#paw-clap" />
          </svg>
        </div>
      ...
      <style jsx>{`
        ...
        .paw-button i {
          ...
          opacity: var(--o, 0);
          background: var(--b);
          transform: translate(var(--x), var(--y)) rotate(var(--r)) scale(var(--s, 1));
          z-index: 4;
          pointer-events: none;
        }
